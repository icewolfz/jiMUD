 ...
 */
Grid.prototype.isWalkableAt = function(x, y, d) {
    return this.isInside(x, y) && (this.nodes[y][x].walkable & d) == d;
};
...
Grid.prototype.getNeighbors = function(node, allowDiagonal, dontCrossCorners) {
    ...
    // ↑
    if (this.isWalkableAt(x, y, 128) && this.isInside(x, y-1, 8)) {
        neighbors.push(nodes[y - 1][x]);
        s0 = true;
    }
    // →
    if (this.isWalkableAt(x, y, 32) && this.isInside(x+1, y, 2)) {
        neighbors.push(nodes[y][x + 1]);
        s1 = true;
    }
    // ↓
    if (this.isWalkableAt(x, y, 8) && this.isInside(x, y+1, 128)) {
        neighbors.push(nodes[y + 1][x]);
        s2 = true;
    }
    // ←
    if (this.isWalkableAt(x, y, 2) && this.isInside(x-1, y, 32)) {
        neighbors.push(nodes[y][x - 1]);
        s3 = true;
    }
    ...
    // ↖ nw
    if (d0 && this.isWalkableAt(x, y, 1) && this.isInside(x-1, y-1, 16)) {
        neighbors.push(nodes[y - 1][x - 1]);
    }
    // ↗ ne
    if (d1 && this.isWalkableAt(x, y, 64) && this.isInside(x+1, y-1, 4)) {
        neighbors.push(nodes[y - 1][x + 1]);
    }
    // ↘ se
    if (d2 && this.isWalkableAt(x, y, 16) && this.isInside(x+1, y+1, 1)) {
        neighbors.push(nodes[y + 1][x + 1]);
    }
    // ↙ sw
    if (d3 && this.isWalkableAt(x, y, 4) && this.isInside(x-1, y+1, 64)) {
        neighbors.push(nodes[y + 1][x - 1]);
    }
...
AStarFinder.prototype.findPath = function(startX, startY, endX, endY, grid) {
		...
		if(!grid.isInside(startX,startY)||!grid.isInside(endX,endY))
			return [];
