...
(function (RoomDetails) {
    RoomDetails[RoomDetails["None"] = 0] = "None";
    RoomDetails[RoomDetails["Dock"] = 1] = "Dock";
    RoomDetails[RoomDetails["Pier"] = 2] = "Pier";
    RoomDetails[RoomDetails["Bank"] = 4] = "Bank";
    RoomDetails[RoomDetails["Shop"] = 8] = "Shop";
    RoomDetails[RoomDetails["Hospital"] = 16] = "Hospital";
    RoomDetails[RoomDetails["Bar"] = 32] = "Bar";
    RoomDetails[RoomDetails["Restaurant"] = 64] = "Restaurant";
    RoomDetails[RoomDetails["WaterSource"] = 128] = "WaterSource";
    RoomDetails[RoomDetails["Trainer"] = 256] = "Trainer";
})(RoomDetails = exports.RoomDetails || (exports.RoomDetails = {}));
var RoomExits;
(function (RoomExits) {
    RoomExits[RoomExits["northwest"] = 1] = "northwest";
    RoomExits[RoomExits["north"] = 128] = "north";
    RoomExits[RoomExits["northeast"] = 64] = "northeast";
    RoomExits[RoomExits["west"] = 2] = "west";
    RoomExits[RoomExits["east"] = 32] = "east";
    RoomExits[RoomExits["southwest"] = 4] = "southwest";
    RoomExits[RoomExits["south"] = 8] = "south";
    RoomExits[RoomExits["southeast"] = 16] = "southeast";
    RoomExits[RoomExits["up"] = 512] = "up";
    RoomExits[RoomExits["down"] = 256] = "down";
})(RoomExits = exports.RoomExits || (exports.RoomExits = {}));
var ImportType;
(function (ImportType) {
    ImportType[ImportType["Merge"] = 0] = "Merge";
    ImportType[ImportType["Replace"] = 1] = "Replace";
...
class Rectangle {
    ...
    constructor(x, y, width, height) {
        ...
        this.size = new library_1.Size(width, height);
    }
}
class Point {
    constructor(x, y) {
...
class Mapper extends EventEmitter {
    constructor(canvas) {
        ...
        this._db = new sqlite3.Database(path.join(library_1.parseTemplate("{data}"), "map.sqlite"));
        this._db.serialize(() => {
            ...
            if (this.Mouse.button === 0 && Math.floor(this.Mouse.x / 32) == Math.floor(this.MouseDown.x / 32) && Math.floor(this.Mouse.y / 32) == Math.floor(this.MouseDown.y / 32)) {
                ...
                this.findActiveRoomByCoords(x, y, (room) => {
                    if (this.selected && room && room.ID == this.selected.ID)
                        return;
                    this.emit('room-before-selected', library_1.clone(this.selected));
                    this.selected = room;
                    this.emit('room-selected', library_1.clone(room));
                    if (!this._redraw)
                        this.draw();
                });
            }
    ...
    get splitArea() { return this._splitArea; }
    set fillWalls(value) {
    ...
    get fillWalls() { return this._fillWalls; }
    getMapMousePos(evt) {
    ...
    findActiveRoomByCoords(rx, ry, callback) {
    ...
    draw(canvas, context, ex, callback) {
        if (!canvas)
            canvas = this._canvas;
        if (!context)
                ...
                if (rows) {
                    ...
                    for (var r = 0; r < rl; r++) {
                        if (rooms[rows[r].ID]) {
                            ...
                            rooms[rows[r].ID] = library_1.clone(rows[r]);
    ...
    focusCurrentRoom() {
        if (this.current.ID)
            this.active = library_1.clone(this.current);
    ...
    setCurrent() {
        ...
        this.current = this.sanatizeRoom(library_1.clone(this.selected));
        this.markers = {};
        this.draw();
    }
    setArea(area) {
        ...
        if (this.current.ID !== null && this.current.area == this.active.area) {
            this.active = this.sanatizeRoom(library_1.clone(this.current));
            this.focusActiveRoom();
