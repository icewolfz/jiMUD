...
const fs = require("fs");
var profiles = new profile_1.ProfileCollection();
var currentProfile;
var currentNode;
var watcher;
var fchanged = false;
...
addmenu.append(new MenuItem({
    label: 'Add alias', click() {
        clearButton("#addmenu");
        addItem('Alias', 'aliases', new profile_1.Alias());
    }
}));
addmenu.append(new MenuItem({
    label: 'Add macro', click() {
        clearButton("#addmenu");
        addItem('Macro', 'macros', new profile_1.Macro());
    }
}));
addmenu.append(new MenuItem({
    label: 'Add trigger', click() {
        clearButton("#addmenu");
        addItem('Trigger', 'triggers', new profile_1.Trigger());
    }
}));
addmenu.append(new MenuItem({
    label: 'Add button', click() {
        clearButton("#addmenu");
        addItem('Button', 'buttons', new profile_1.Button());
    }
}));
...
function UpdateButtonSample() {
    ...
    if (icon.startsWith("fa-"))
        ...
        button.html('<img src="' + library_1.parseTemplate(icon) + '" />');
    else
        button.html('<i class="fa fa-question"></i>');
}
exports.UpdateButtonSample = UpdateButtonSample;
function openImage() {
...
function MacroeKeys(item) {
    if (item.key === 0)
        return "None";
    var d = [];
    if ((item.modifiers & profile_1.MacroModifiers.Ctrl) == profile_1.MacroModifiers.Ctrl)
        d.push("Ctrl");
    if ((item.modifiers & profile_1.MacroModifiers.Alt) == profile_1.MacroModifiers.Alt)
        d.push("Alt");
    if ((item.modifiers & profile_1.MacroModifiers.Shift) == profile_1.MacroModifiers.Shift)
        d.push("Shift");
    if ((item.modifiers & profile_1.MacroModifiers.Meta) == profile_1.MacroModifiers.Meta) {
...
function getEditorValue(editor, style) {
    if (editors[editor]) {
        if (style == profile_1.ItemStyle.Script)
...
exports.updateProfileChecks = updateProfileChecks;
...
function canPaste() {
    if (_clip) {
        if (_clip.data.length === 1 && _clip.type == "macro") {
            var r = library_1.FilterArrayByKeyValue(currentProfile.macros, "key", _clip.data[0].key);
            r = library_1.FilterArrayByKeyValue(r, "modifiers", _clip.data[0].modifiers);
            if (r.length == 0)
                return true;
        }
        else
...
exports.UpdatePaste = UpdatePaste;
function doUndo() {
}
            ...
            switch (t) {
                ...
                case "alias":
                    ...
                    if (editors['AliasValue']) {
                        ...
                        if (currentProfile.aliases[currentNode.dataAttr.index].style == profile_1.ItemStyle.Script)
                ...
                case "macro":
                    ...
                    if (editors['MacroValue']) {
                        ...
                        if (currentProfile.macros[currentNode.dataAttr.index].style == profile_1.ItemStyle.Script)
                ...
                case "trigger":
                    ...
                    if (editors['TriggerValue']) {
                        ...
                        if (currentProfile.triggers[currentNode.dataAttr.index].style == profile_1.ItemStyle.Script)
                ...
                case "button":
                    ...
                    if (editors['ButtonValue']) {
                        ...
                        if (currentProfile.buttons[currentNode.dataAttr.index].style == profile_1.ItemStyle.Script)
...
function init() {
    $(document).ready(() => {
        var options = settings_1.Settings.load(path.join(library_1.parseTemplate("{data}"), 'settings.json'));
        _never = options.profiles.askoncancel;
        var p = path.join(library_1.parseTemplate("{data}"), "profiles");
        if (!fs.existsSync(p)) {
            profiles.add(profile_1.Profile.Default);
        }
        else {
            profiles.load(p);
        ...
        $("#tree").contextmenu((event) => {
            ...
            c.popup(electron_1.remote.getCurrentWindow());
        });
        ...
        $(window).on('resize', () => {
            if ($('#editor').outerWidth() < 300 && $("#sidebar").outerWidth() > 202) {
                ...
                electron_1.ipcRenderer.send('setting-changed', { type: 'profiles', name: 'split', value: document.body.clientWidth - 300 });
            }
        });
        $(document).mouseup(function (e) {
            if (dragging) {
                if (e.pageX < 200) {
                    ...
                    $('#editor').css("left", 202);
                    electron_1.ipcRenderer.send('setting-changed', { type: 'profiles', name: 'split', value: 202 });
                }
                    ...
                    electron_1.ipcRenderer.send('setting-changed', { type: 'profiles', name: 'split', value: e.pageX + 2 });
                }
                $('#ghostbar').remove();
        ...
        $("#addMenu").click(function () {
            ...
            addmenu.popup(electron_1.remote.getCurrentWindow(), x, y);
        });
        ...
        $("#MacroKey").keyup(function (e) {
            ...
            e.stopPropagation();
            var c = [];
            var mod = profile_1.MacroModifiers.None;
            if (e.ctrlKey) {
                c.push("Ctrl");
                mod |= profile_1.MacroModifiers.Ctrl;
            }
            if (e.altKey) {
                c.push("Alt");
                mod |= profile_1.MacroModifiers.Alt;
            }
            if (e.shiftKey) {
                c.push("Shift");
                mod |= profile_1.MacroModifiers.Shift;
            }
            if (e.metaKey) {
                mod |= profile_1.MacroModifiers.Meta;
                if (process.platform == "darwin")
                    c.push("Cmd");
                else
                    c.push("Win");
            }
            if (library_1.keyCodeToChar[e.which])
                c.push(library_1.keyCodeToChar[e.which]);
            else
                return false;
            $("#MacroKey").val(c.join("+"));
        ...
        $(".btn-adv").click(function () {
            ...
            if (state) {
                ...
                electron_1.ipcRenderer.send('setting-changed', { type: 'profiles', name: editor + 'Advanced', value: true });
            }
            else {
                panels.css('display', '');
                ...
                electron_1.ipcRenderer.send('setting-changed', { type: 'profiles', name: editor + 'Advanced', value: false });
            }
        });
        ...
        loadActions(library_1.parseTemplate(path.join("{assets}", "actions")), "");
        ...
        window.onbeforeunload = function () {
            if (close || _never || (_undo.length == 0 && !updateCurrent()))
                return;
            var choice = dialog.showMessageBox(electron_1.remote.getCurrentWindow(), {
                type: 'warning',
            ...
            if (choice === 2) {
                electron_1.ipcRenderer.send('setting-changed', { type: 'profiles', name: 'askoncancel', value: false });
                choice = 0;
            }
            if (choice === 0)
                return;
    ...
    if (_undo.length > 0)
        dialog.showMessageBox(electron_1.remote.getCurrentWindow(), {
            ...
            if (response == 0) {
                fchanged = false;
                resetUndo();
                profiles = new profile_1.ProfileCollection();
...
function trashProfiles(p) {
    if (_remove.length > 0) {
        for (var r = 0, rl = _remove.length; r < rl; r++) {
            electron_1.shell.moveItemToTrash(path.join(p, _remove[r] + ".json"));
            /*
...
exports.trashProfiles = trashProfiles;
    ...
    if (fchanged)
        dialog.showMessageBox(electron_1.remote.getCurrentWindow(), {
            ...
            if (response == 0) {
                ...
                profiles.save(p);
                trashProfiles(p);
                electron_1.ipcRenderer.send('reload-profiles');
...
exports.updateUndoState = updateUndoState;
...
function doImport(node) {
    ...
    var profile = profiles.items[node.dataAttr.profile];
    var m = profile_1.Profile.DefaultMacros;
