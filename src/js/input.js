...
class input extends EventEmitter {
    constructor(client) {
        ...
        this.client = client;
        this._tests = new test_1.Tests(client);
    ...
    executeScript(txt) {
        ...
        var stackingchar = this.client.options.commandStackingChar;
        var state = 0;
        var idx = 0, c, r;
        var tl = txt.length;
        var fun = "";
        ...
        var arg = "";
        var raw;
        var pd = this.client.options.parseDoubleQuotes;
        var ps = this.client.options.parseSingleQuotes;
        for (; idx < tl; idx++) {
            c = txt.charAt(idx);
            switch (state) {
                case 1:
        ...
        switch (fun.toLowerCase()) {
            ...
            case "idletime":
                if (!this.client.lastSendTime)
                    ...
                    this.client.echo("You have been idle: " + library_1.getTimeSpan(Date.now() - this.client.lastSendTime), -7, -8, true, true);
                return null;
            case "connect":
            case "connecttime":
                if (!this.client.connectTime)
                    ...
                    this.client.echo("You have been connected: " + library_1.getTimeSpan(Date.now() - this.client.connectTime), -7, -8, true, true);
                return null;
            case "version":
            case "ve":
            ...
            case "al":
                if (args.length === 0)
                    ...
                    if (/^\d+$/.exec(n)) {
                        ...
                        if (!f) {
                            tmp = new profile_2.Alias(n, args);
                            items.push(f);
    ...
    parseOutgoing(text, eAlias, stacking) {
        ...
        if (text === null || tl === 0)
            return text;
        var str = "";
        var alias = "", AliasesCached;
        var state = 0;
        var aliases = this.client.aliases;
        var stackingchar = this.client.options.commandStackingChar;
        var spChar = this.client.options.speedpathsChar;
        var ePaths = this.client.options.enableSpeedpaths;
        var args = [];
        var arg = "";
        var findAlias = true;
        var out = "";
        var a, c, al;
        var idx = 0, start = true;
        var pd = this.client.options.parseDoubleQuotes;
        var ps = this.client.options.parseSingleQuotes;
        if (eAlias == null)
            ...
            switch (state) {
                ...
                default:
                    if (ePaths && start && c === spChar) {
                        ...
                        AliasesCached = library_1.FilterArrayByKeyValue(aliases, "pattern", alias);
    ...
    ParseString(text, args, named, append) {
        if (text == null)
            return "";
        var tl = text.length;
        if (tl === 0)
    ...
    ProcessMacros(keycode, alt, ctrl, shft, meta) {
        ...
        var macros = library_1.FilterArrayByKeyValue(this.client.macros, "key", keycode);
        var m = 0, ml = macros.length;
        for (; m < ml; m++) {
            if (!macros[m].enabled)
                continue;
            if (alt === ((macros[m].modifiers & profile_1.MacroModifiers.Alt) != profile_1.MacroModifiers.Alt))
                continue;
            if (ctrl === ((macros[m].modifiers & profile_1.MacroModifiers.Ctrl) != profile_1.MacroModifiers.Ctrl))
                continue;
            if (shft === ((macros[m].modifiers & profile_1.MacroModifiers.Shift) != profile_1.MacroModifiers.Shift))
                continue;
            if (meta === ((macros[m].modifiers & profile_1.MacroModifiers.Meta) != profile_1.MacroModifiers.Meta))
                continue;
    ...
    ProcessPath(str) {
        if (str.length === 0)
            return "";
        var pPaths = this.client.options.parseSpeedpaths;
        var out = "";
        var state = 0;
        var cmd = "";
        var num = "";
    ...
    ;
    ExecuteTriggers(type, raw, frag, ret) {
        if (raw == null)
            return raw;
        if (ret == null)
            ret = false;
        ...
        if (this._TriggerCache === null) {
            ...
            this._TriggerCache.sort(library_1.SortArrayByPrioity);
        }
        var t = 0, tl = this._TriggerCache.length;
        for (; t < tl; t++) {
            if (typeof this._TriggerCache[t].type != "undefined" && this._TriggerCache[t].type != type)
                continue;
            if (frag && !this._TriggerCache[t].triggerprompt)
                continue;
            ...
            if (this._TriggerCache[t].verbatim || (typeof this._TriggerCache[t].regex != "undefined" && !this._TriggerCache[t].regex)) {
                if (raw != this._TriggerCache[t].pattern)
                    continue;
                if (ret)
    ...
    ExcuteTrigger(trigger, args, r, idx) {
        if (r == null)
            r = false;
        if (!trigger.enabled)
            return "";
