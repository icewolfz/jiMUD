...
class Client extends EventEmitter {
    constructor(display, command) {
        ...
        this.character = null;
        this.commandCharacter = null;
        this.connecting = false;
        this.profiles = new profile_1.ProfileCollection();
        this.connectTime = 0;
        this.lastSendTime = 0;
        this.defaultTitle = 'jiMUD';
        ...
        if (typeof command === 'string')
            ...
            this.commandInput = command;
        this._MSP = new msp_1.MSP();
        this._MSP.on('playing', (data) => {
            if (this.enableDebug)
                this.emit('MSP ' + (data.type ? 'Music' : 'Sound') + ' Playing ' + data.file + ' for ' + data.duration);
            if (!this.options.notifyMSPPlay)
                return;
            this.echo((data.type ? 'Music' : 'Sound') + ' Playing ' + data.file + ' for ' + data.duration, ansi_1.AnsiColorCode.InfoText, ansi_1.AnsiColorCode.InfoBackground, true, true);
        });
        ...
        this.commandInput.focus();
        this.telnet = new telnet_1.Telnet();
        ...
        this.telnet.on('connecting', () => {
            ...
            this.echo("Trying to connect to " + this.host + ":" + this.port + ".", ansi_1.AnsiColorCode.InfoText, ansi_1.AnsiColorCode.InfoBackground, true, true);
        });
        this.telnet.on('connect', () => {
            this.connecting = false;
            this.echo("Connected...", ansi_1.AnsiColorCode.InfoText, ansi_1.AnsiColorCode.InfoBackground, true, true);
        ...
        this.telnet.on('receiveOption', (data) => {
            this._MSP.processOption(data);
            this.emit('receivedOption', data);
        });
        this.telnet.on('close', () => {
            this.connecting = false;
            this.echo("Connection closed to " + this.host + ":" + this.port + ".", ansi_1.AnsiColorCode.InfoText, ansi_1.AnsiColorCode.InfoBackground, true, true);
            this.connectTime = 0;
            this.lastSendTime = 0;
            this._MSP.reset();
            this.emit('closed');
        ...
        this.parser.on('addLine', (data) => {
            data.raw = library_1.stripHTML(data.line);
    ...
    loadProfiles() {
        ...
        if (!fs.existsSync(p)) {
            this.profiles.add(profile_1.Profile.Default);
            return;
        }
    ...
    loadOptions() {
        this.options = settings_1.Settings.load(path.join(library_1.parseTemplate("{data}"), 'settings.json'));
        ...
        this._MSP.savePath = library_1.parseTemplate(this.options.soundPath);
    ...
    saveOptions() {
        this.options.save(path.join(library_1.parseTemplate("{data}"), 'settings.json'));
    }
    UpdateFonts() {
    ...
    ;
    parseInternal(txt, remote, force) {
    ...
    error(err) {
        ...
        if (err.error === null || typeof err.error == "undefined")
            this.echo("Error: Unknown.", ansi_1.AnsiColorCode.ErrorText, ansi_1.AnsiColorCode.ErrorBackground, true, true);
        else if (typeof err.error == "string" && err.error.length === 0)
            this.echo("Error: Unknown.", ansi_1.AnsiColorCode.ErrorText, ansi_1.AnsiColorCode.ErrorBackground, true, true);
        else if (err.error instanceof ReferenceError) {
            if (err.error.lineNumber)
                this.echo("Error: File: " + err.error.fileName + ", Line: " + err.error.lineNumber + ", " + err.error.message + ".", ansi_1.AnsiColorCode.ErrorText, ansi_1.AnsiColorCode.ErrorBackground, true, true);
            else
                this.echo("Error: " + err.error + ".", ansi_1.AnsiColorCode.ErrorText, ansi_1.AnsiColorCode.ErrorBackground, true, true);
        }
        else if (err.error instanceof Error)
            this.echo("Error: " + err.error.name + " - " + err.error.message + ".", ansi_1.AnsiColorCode.ErrorText, ansi_1.AnsiColorCode.ErrorBackground, true, true);
        else if (err.error.message)
            this.echo("Error: " + err.error.message + ".", ansi_1.AnsiColorCode.ErrorText, ansi_1.AnsiColorCode.ErrorBackground, true, true);
        else
            this.echo("Error: " + err.error + ".", ansi_1.AnsiColorCode.ErrorText, ansi_1.AnsiColorCode.ErrorBackground, true, true);
    ...
    echo(str, fore, back, newline, forceline) {
        ...
        if (forceline == null)
            forceline = false;
        if (fore == null)
            fore = ansi_1.AnsiColorCode.LocalEcho;
        if (back == null)
            back = ansi_1.AnsiColorCode.LocalEchoBack;
    ...
    printInternal(txt, newline, remote) {
    ...
    sendCommand(txt) {
        ...
        if (data === null || typeof data == "undefined")
            return;
        if (data.handled || data.value === null || typeof data.value == "undefined")
            return;
    ...
    ;
    get WindowSize() {
        return new library_1.Size(this.WindowWidth, this.WindowHeight);
    }
    get WindowWidth() {
    ...
    parseOutgoing(text, eAlias, stacking) {
